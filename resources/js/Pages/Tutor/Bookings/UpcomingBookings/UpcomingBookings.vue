        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
            ...
            <div class="flex items-center space-x-2 flex-grow sm:flex-grow-0">
                <div class="relative flex-grow sm:flex-grow-0">
                    ...
                    <svg class="w-4 h-4 text-gray-400 absolute left-2 top-1/2 transform -translate-y-1/2" fill="none"
            ...
            <div class="flex items-center border border-gray-300 dark:border-gray-600 rounded text-sm">
                <button v-for="view in views" :key="view.value" @click="currentView = view.value" class="px-3 py-1"
                    :class="{
        ...
        <div v-if="currentView === 'weekly'" class="border border-l border-gray-200 dark:border-gray-700">
            ...
            <div class="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700">
                <div v-for="day in weekDays" :key="`header-${day.toISOString()}`"
                    ...
                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ formatDate(day, { day: 'numeric', month: 'short' }) }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ formatDate(day, { weekday: 'short' }) }}</div>
                </div>
            </div>
            <!-- Content Area -->
            <div class="grid grid-cols-7 min-h-[50vh]">
                <div v-for="day in weekDays" :key="`content-${day.toISOString()}`"
                    ...
                    :class="{ 'bg-blue-50 dark:bg-blue-900/10': isToday(day) }">
                    <div class="bg-gray-50 dark:bg-gray-700/50 rounded p-2 text-center text-xs text-gray-500 dark:text-gray-400">
                        No sessions
                    </div>
                </div>
            </div>
...
.bg-gray-50.dark\:bg-gray-700\/30 {
    ...
    background-image: repeating-linear-gradient(
        ...
        transparent,
        transparent 5px,
        rgba(0, 0, 0, 0.02) 5px, /* Light mode pattern */
...
.dark .bg-gray-50.dark\:bg-gray-700\/30 {
     background-image: repeating-linear-gradient(
        ...
        transparent,
        transparent 5px,
        rgba(255, 255, 255, 0.03) 5px, /* Dark mode pattern */
