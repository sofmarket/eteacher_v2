    ...
    <div class="w-full bg-white border-t" id="availability-tab">
        <div class="w-full max-w-7xl mx-auto py-15 px-15">
            ...
            <div class="bg-white rounded-lg py-4 h-[350px] relative" ref="slotsContainer">
                <div class="flex items-start justify-between">
                    ...
                    <div class="grid grid-cols-1 gap-4 w-full sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7">
                        <div v-for="day in weekDays" :key="day.date.toISOString()" class="text-center"
                            v-memo="[day.date, day.isSelected, day.isBeforeToday, slotsByDate[day.date.toISOString().split('T')[0]]]">
                            ...
                            <div class="w-full">
                                <div v-if="slotsByDate[day.date.toISOString().split('T')[0]]?.length > 0"
                                    class="space-y-2">
                                    <div v-for="slot in slotsByDate[day.date.toISOString().split('T')[0]]"
                                        ...
                                        <div class="text-center py-1 select-none" @click="bookSlot(slot)">
                                            ...
                                            <div v-if="slot.available_spaces" class="text-xs opacity-75">{{
                                                slot.available_spaces }}/{{ slot.spaces }}</div>
                                        </div>
                                    </div>
                                </div>
                                <div v-else
                                    class="px-3 py-4 rounded-lg text-sm font-medium transition-colors select-none"
                                    :class="{
                ...
                <div class="absolute top-0 right-0 left-0 bottom-0 bg-transparent" v-if="precessing">
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <div class="w-10 h-10">
                            <svg width="60" height="60" viewBox="0 0 44 44">
                                ...
                                <circle cx="22" cy="22" r="20" fill="none" stroke="#1447e6" stroke-width="4"
                                    stroke-dasharray="125.6" stroke-dashoffset="125.6">
                                    <animate attributeName="stroke-dashoffset" values="125.6;0" dur="1s"
