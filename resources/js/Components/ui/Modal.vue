<template>
  <teleport to="body">
    <div v-if="modalActive" class="fixed inset-0 z-[9999] overflow-y-auto bg-gray-400/50 backdrop-blur-[32px]">
      <!-- wrapper gives spacing -->
      <div class="min-h-screen flex justify-center p-5">
        <!-- modal -->
        <div class="relative w-full max-w-[600px] rounded-lg bg-white py-8 px-6 dark:bg-gray-900">

          <!-- header -->
          <div class="flex items-center justify-between mb-8">

            <h4 class="text-gray-800 text-xl dark:text-white/90">
              {{ title }}
            </h4>

            <button @click="$emit('close')"
              class="flex h-7 w-7 items-center justify-center rounded-full bg-gray-100 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
          ...
          <!-- body (auto height = content height) -->
          <div>
            <slot name="body"></slot>
          </div>
          ...
          <div class="flex items-center justify-end w-full gap-3 mt-8">
            <button @click="$emit('reset'), $emit('close')" type="button"
